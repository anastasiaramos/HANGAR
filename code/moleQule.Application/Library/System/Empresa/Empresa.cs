...
namespace moleQule.Library.Application
{
    ...
    public class Empresa : BusinessBaseEx<Empresa>, moleQule.Library.ISchema
    {
        ...
        {
            Serial = SerialInfo.GetNext(typeof(Empresa));
        ...
        public static Empresa Get(long oid)
        {
            ...
            criteria.Query = Empresa.SELECT(oid);

            Empresa.BeginTransaction(criteria.Session);
            return DataPortal.Fetch<Empresa>(criteria);
        }

        public virtual EmpresaInfo GetInfo() { return GetInfo(true); }
        
        public virtual EmpresaInfo GetInfo(bool get_childs)
        {
            if (!CanGetObject())
                throw new System.Security.SecurityException(
                  moleQule.Library.Resources.Messages.USER_NOT_ALLOWED);

            return new EmpresaInfo(this, get_childs);
        }
