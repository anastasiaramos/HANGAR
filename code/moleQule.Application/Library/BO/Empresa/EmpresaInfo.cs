...
{
    [Serializable()]
    public class EmpresaInfo : ReadOnlyBaseEx<EmpresaInfo>, moleQule.Library.ISchemaInfo, IAgenteHipatia
    {
        ...
        public static string GetLogoPath(long oid_schema)
        {
            EmpresaInfo empresa = Get(oid_schema, false);
            return Controler.LOGOS_EMPRESAS_PATH + empresa.Logo;
        }
        ...
        private EmpresaInfo() { /* require use of factory methods */ }
        ...
        internal EmpresaInfo(Empresa source, bool copy_childs)
        {
            CopyValues(source);

            if (copy_childs)
            {
                _contactos = (source.Contactos != null) ? ContactoEmpresaList.GetChildList(source.Contactos) : null;
            }
        }
